  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Add/Edit Pengabdian</h4>
      </div>
      <div class="modal-body form-horizontal">
      	...
      	<form class="form-horizontal" id="edit-form" method="post" enctype="multipart/form-data">
		  	...
		  	<input type="hidden" name="pengabdianID" id="pengabdianID">
		 </form>
  	...
  	<div class="modal-dialog">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	      		<input type="hidden" name="rowid" id="rowid">
	      		<p>Hapus Pengabdian yang dipilih?</p>
	      <div class="modal-footer">
...
<script type="text/javascript">
  ...
  $(function () {   
	...
	function reloaddt() {
	    ...
	    table = $('#tbldata').DataTable({
	      "pageLength": 10,
	       ...
	       "bInfo": false,
	       "ajax":{  
	            url:"<?php echo base_url() . 'pengabdian/fetch_pengabdian/'; ?>",
	            type:"POST",
	            "data": {
	                "period": $('#period').val()
	            }
	      ...
	      },
	      {
	          ...
	          "data": null,
	          "render": function ( data, type, row, meta ) {
	            return '<a href="<?php echo site_url(); ?>pengabdian/showfile/'+data[0]+'/'+data[1]+'" target="_blank">'+data[5]+'</a>';
	          }
	      },
	...
	function edititem(pengabdianID) {
		var s = $('#idxval').val();
		var temp = table.row(s).data();
		$('#tanggaledits').val(temp[3]);
		$('#keterangan').val(temp[4]);
		$('#namafile').val(temp[5]);

		$('#pengabdianID').val(pengabdianID);
		$('#delbut').show();
		$('#modaldata').modal('show');
	}
	...
	function deleterow() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>pengabdian/deletepengabdian",
			data: dataString,
	...
	function addrow() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>pengabdian/savepengabdian",
			...
			success: function(data){
			   ...
			   //alert('d:'+d);
			    ...
			    if($.isNumeric( d['stt'] )){
					...
					if($('#period').val() == null) {
						location.href = '<?php echo base_url(); ?>pengabdian/';
					} else {
						 reloaddt();
					}
