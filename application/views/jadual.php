        ...
        <div class="col-xs-12" style="padding-left:5px; padding-right:5px;">         
          ...
          <div class="box">
            ...
            <div class="box-body table-responsive">
				<table id="tbljadual" class="table table-bordered table-striped table-hover" style="width:100% !important;">
	                <thead>
		                <tr>
		                  <th>ID</th>
		                  <th>Nama Jadual</th>
	                ...
	                <?php if(!empty($jadual)) { ?>
	                <?php foreach ($jadual as $rw) { ?>
	                	<tr style="font-size:small;">
	                		<td><a href="#<?php echo $detailcount;?>" onclick="edititem('<?php echo $rw['jadualID'];?>');"><?php echo $rw['jadualID'];?></a></td>
	                		<td><?php echo $rw['jadualNama'];?></td>
...
<div class="modal fade" id="modaljadual" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">Add/Edit Jadual</h4>
      </div>
      <div class="modal-body">
      	...
      	<input type="hidden" id="jadualID" name="jadualID">      		
		  	<div class="form-group">
		    	<label for="jadualNama" class="col-sm-3 control-label">Nama Jadual</label>
		    	<input type="text" class="form-control input-sm" id="jadualNama" name="jadualNama" 
		      		value="">
		  	</div>	 
		  	<div class="form-group">
		  	...
		  	<input type="hidden" name="rowidjadual" id="rowidjadual">
      </div>
...
<script type="text/javascript">
  ...
  $(function () {   
    ...
    $('#tbljadual tfoot th').each( function () {
          var title = $(this).text();
          $(this).html( '<input type="text" placeholder="Search '+title+'" style="width:100%;" />' );
      } );

    table = $('#tbljadual').DataTable({
    	"responsive": true,
    	"paging": false,
    	"dom": 'lrtip',
    	"columnDefs": [
    ...
    $('#tbljadual tfoot tr').appendTo('#tbljadual thead');
    $('#tbljadual tbody').on( 'mousedown', 'tr', function () {
  	...
  	function addnew() {
		...
		$('#delbut').hide();
		$('#modaljadual').modal('show');
		
	}
	function edititem(jadualID) {
		
		var s = $('#idxval').val();
		var temp = table.row(s).data();
		$('#jadualNama').val(temp[1]);
		if(temp[3] == 0) {
	...
	function addrow() {
		...
		var dataString = $('#jadualNama').serializeArray();
		dataString.push({name: 'jadualID', value: n});
		dataString.push({name: 'jadualNama', value: p});
		dataString.push({name: 'isAktif', value: a});
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>jadual/savejadual",
			data: dataString,
			success: function(data){
			    ...
			    if($.isNumeric( d['stt'] )){
			        ...
			        if(v==false){
						...
						temp[1] = $('#jadualNama').val();
						temp[2] = stra;
						temp[3] = a;
						$('#tbljadual').dataTable().fnUpdate(temp,r,undefined,true);	
				    }
			    	$('#btncontinuehadir').show();
    				$('#btnloadhadir').hide();
					$('#modaljadual').modal('hide');
			    }else{
			    	$('#btncontinuehadir').show();
    				$('#btnloadhadir').hide();
